...
# endif
	...
	/**
		...
		* @param z The z depth,
			controls if the image is on the fore or background.
	...
	/**
		...
		* @param modifier The modifier key that was pressed,
	...
	/**
		...
		* @param enabled If true the image is drawn onto the screen,
			else it's not.
	...
	before calling `mlx_init` as they define the behaviour of MLX42.typedef enum mlx_settings {
		MLX_STRETCH_IMAGE = 0,
	...
	/**
		...
		*/
	typedef void		(*mlx_scrollfunc)(double xdelta, double ydelta,
				void *param);
	...
	/**
		...
		* @param[in] param Additional parameter to pass on to the function.
		*/
	typedef void		(*mlx_cursorfunc)(double xpos, double ypos,
				void *param);
	...
	/**
		...
		* @param[in] param Additional parameter to pass on to the function.
		*/
	typedef void		(*mlx_resizefunc)(int32_t width, int32_t height,
				void *param);
	...
	/**
		...
		* @param[in] value Settings value to determine the state of the setting. Can be a boolean or an enum
			/ macro.
		*/
	...
	/**
		...
		* This is not the same as terminate,
			this simply tells MLX to close the window.
		*
	...
	/**
		...
		* mlx_close_window is called,
			meaning it will loop until the user requests that
	...
	/**
		...
		*
		* NOTE: In MacOS this function does nothing,
			you should use the bundles icon to set the dock bar icon.
	...
	/**
		...
		*
		* @param[in] index Normally 0, in case of multiple windows,
			can be specified
		* @param[in] width The width of the window.
		* @param[in] height The height of the window.
		*/
	...
	/**
		...
		*
		* Pass
			-1 for no limit to any of the min/max parameters to ignore that boundary.
	...
	/**
		* Returns the current, relative, mouse cursor position on the window,
			starting
	...
	/**
		...
		* @param[in] cursor The cursor object to display,
			if null default cursor is selected.
		*/
	...
	/**
		* This function sets the scroll callback,
			which is called when a scrolling
	...
	/**
		* This function sets the key callback,
			which is called when a key is pressed
	...
	/**
		* This function sets the close callback,
			which is called in attempt to close
	...
	/**
		* This function sets the resize callback,
			which is called when the window is
		* resized
		*
		* @param[in] mlx The MLX instance handle.
	...
	/**
		...
		* @return Pointer to the image buffer,
			if it failed to allocate then NULL.
		*/
	...
	/**
		...
		* instances it might have. Additionally,
			just as extra measures sets all the
		* data to NULL.
		*
